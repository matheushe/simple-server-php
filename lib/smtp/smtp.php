...
class SMTP extends MIME {
	...
	function addto($adrr, $name = ''){
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				...
				}else trigger_error('Already exists ('.$adrr.'), on class SMTP::addto()', 512);
			}else trigger_error('Invalid 1\'st parameter value ('.$adrr.'), on class SMTP::addto()', 512);
	...
	function send($subject){
		...
		if(is_string($subject)){
			...
			if($subject != ""){
				$this->_subject = sanitizar($subject);//$this->qpheader($subject);
